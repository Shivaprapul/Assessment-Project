/**
 * Facilitator Quest Runner Page
 * 
 * Runs a facilitator quest (mini_game, reflection, or choice_scenario).
 * Reuses assessment runner patterns for mini games.
 * 
 * @module app/(student)/facilitator/quests/[questId]
 */
...
export default function QuestRunnerPage({ params }: { params: Promise<{ questId: string }> }) {
  ...
  const startQuest = async (qId: string) => {
    try {
      const response = await fetch(`/api/facilitator/quests/${qId}/start`, {
        method: 'POST',
        credentials: 'include',
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/facilitator/quests/${questId}/submit`, {
        method: 'POST',
        headers: {
      ...
      const data = await response.json();
      if (data?.success) {
        // Map API response to UI format (facilitator uses coachingInsight)
        const coachingInsight = data.data?.coachingInsight || {};
        const mappedInsight = {
          strength: coachingInsight.strengthObserved || coachingInsight.strength || 'You completed the quest!',
          growth: coachingInsight.improvementTip || coachingInsight.growth || 'Keep training to reach your goal',
          evidence: Array.isArray(coachingInsight.evidence) ? coachingInsight.evidence : [],
          skillTags: Array.isArray(coachingInsight.skillSignals) ? coachingInsight.skillSignals : 
                    Array.isArray(coachingInsight.skillTags) ? coachingInsight.skillTags : [],
          goalAlignment: coachingInsight.goalAlignment || '',
        };
        ...
        const resultsData = {
          ...
          coachingInsight: mappedInsight,
          goalReadiness: data.data?.goalReadiness || null,
        };
        
        // Navigate to results page with results data in URL state
        const encodedData = encodeURIComponent(JSON.stringify(resultsData));
        router.push(`/facilitator/quests/${questId}/results?data=${encodedData}`);
      } else {
        throw new Error('Invalid response from server');
      }
    } catch (err: any) {
